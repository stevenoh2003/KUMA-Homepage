...
export default async function handler(req, res) {
  ...
  form.parse(req, async (err, fields) => {
    ...
    const currentTitle = Array.isArray(fields.currentTitle) ? fields.currentTitle[0] : fields.currentTitle;
    ...
    const isPublic = String(Array.isArray(fields.isPublic) ? fields.isPublic[0] : fields.isPublic).toLowerCase() === "true";
    const thumbnailName = Array.isArray(fields.thumbnailName) ? fields.thumbnailName[0] : fields.thumbnailName;
    const thumbnailType = Array.isArray(fields.thumbnailType) ? fields.thumbnailType[0] : fields.thumbnailType;

    // Ensure required fields are provided
    if (!currentTitle || !newTitle || !newDescription) {
      console.error("Missing required fields:", { currentTitle, newTitle, currentDescription, newDescription });
    ...
    try {
      ...
      // Update the post title, description, and visibility
